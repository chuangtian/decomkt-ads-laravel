...
class ReputationController extends Controller
{
    public function store(Request $request, StoreContext $storeContext): RedirectResponse
    {
        ...
        match ($data['entity']) {
            'review' => DB::table('ReputationReview')->insert(['id' => $id, 'storeId' => $storeId, 'platform' => $data['platform'] ?? 'WEBSITE', 'content' => $data['content'], 'star' => $data['star'] ?? 5, 'sentiment' => $data['sentiment'] ?? 'POSITIVE', 'replied' => false, 'source' => 'manual', 'createdAt' => now(), 'updatedAt' => now()]),
            'reddit' => DB::table('RedditPost')->insert(['id' => $id, 'storeId' => $storeId, 'title' => $data['title'] ?? 'Reddit 帖子', 'body' => $data['content'], 'upvotes' => 0, 'views' => 0, 'commentCount' => 0, 'sentiment' => $data['sentiment'] ?? 'NEUTRAL', 'replied' => false, 'source' => 'manual', 'createdAt' => now(), 'updatedAt' => now()]),
            'risk' => DB::table('ReputationRisk')->insert(['id' => $id, 'storeId' => $storeId, 'description' => $data['content'], 'level' => $data['level'] ?? 'MEDIUM', 'platform' => $data['platform'] ?? '综合', 'status' => 'OPEN', 'source' => 'manual', 'createdAt' => now(), 'updatedAt' => now()]),
            'resource' => DB::table('ReputationResource')->insert(['id' => $id, 'storeId' => $storeId, 'description' => $data['content'], 'type' => $data['platform'] ?? 'OTHER', 'priority' => $data['priority'] ?? 'MEDIUM', 'createdAt' => now(), 'updatedAt' => now()]),
        };

        return back()->with('success', '记录已添加');
    }

    public function update(Request $request, string $entity, string $id, StoreContext $storeContext): RedirectResponse
    {
        ...
        DB::table($table)->where('storeId', $storeContext->id())->where('id', $id)->update([...$data, 'updatedAt' => now()]);

        return back()->with('success', '记录已更新');
    }

    public function destroy(string $entity, string $id, StoreContext $storeContext): RedirectResponse
    {
        $table = ['review' => 'ReputationReview', 'reddit' => 'RedditPost', 'risk' => 'ReputationRisk', 'resource' => 'ReputationResource'][$entity] ?? null;
        abort_unless($table, 404);
        DB::table($table)->where('storeId', $storeContext->id())->where('id', $id)->delete();

        return back()->with('success', '记录已删除');
    }

    public function analyze(StoreContext $storeContext): RedirectResponse
    {
        $storeId = $storeContext->id();
        ...
        $negativeReviews = DB::table('ReputationReview')
            ...
            ->groupBy('platform')
            ->get();
        $negativeReddit = DB::table('RedditPost')->where('storeId', $storeId)->where('sentiment', 'NEGATIVE')->count();

        DB::transaction(function () use ($week, $negativeReviews, $negativeReddit, $storeId) {
            DB::table('ReputationRisk')->where('storeId', $storeId)->where('source', 'auto')->where('week', $week)->delete();
            foreach ($negativeReviews as $item) {
                DB::table('ReputationRisk')->insert([
                    'id' => (string) Str::uuid(), 'storeId' => $storeId,
    ...
    public function saveWeeklyReport(Request $request, StoreContext $storeContext): RedirectResponse
    {
        $data = $request->validate([
        ...
        $week = (int) now()->isoWeek();
        $storeId = $storeContext->id();
        $reviews = DB::table('ReputationReview')->where('storeId', $storeId);
        ...
        DB::table('ReputationWeeklyReport')->updateOrInsert(
            ['storeId' => $storeId, 'year' => $year, 'week' => $week],
            [
                'id' => DB::table('ReputationWeeklyReport')->where('storeId', $storeId)->where('year', $year)->where('week', $week)->value('id') ?: (string) Str::uuid(),
                ...$data, 'overallScore' => collect([$tpAvg, $websiteAvg])->filter(fn ($value) => $value !== null)->avg(),
                'tpAvg' => $tpAvg, 'websiteAvg' => $websiteAvg,
                'redditPosts' => DB::table('RedditPost')->where('storeId', $storeId)->count(),
