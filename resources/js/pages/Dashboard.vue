...
<template>
    ...
    <div class="deco-page">
        <div class="deco-header">
            ...
            <input class="deco-input deco-date" value="2026-08-05　-　2026-08-11" aria-label="日期范围" />
        </div>

        <section class="deco-card mb-4 border-t-2 !border-t-lime-500">
            <div class="mb-5 flex items-center justify-between">
                <h2 class="deco-card-title !mb-0">Shopify 独立站</h2>
                <Link href="/ecommerce/shopify" class="text-sm text-lime-400">查看详情 →</Link>
            </div>
            <div class="grid grid-cols-2 gap-8 md:grid-cols-4">
                <div v-for="label in ['GMV','订单数','客单价','日均 GMV']" :key="label">
                    <p class="deco-metric-label">{{ label }}</p>
                    <p class="mt-3 text-2xl font-bold" :class="label === 'GMV' ? 'text-lime-400' : 'text-white'">—</p>
                </div>
            </div>
        </section>
        ...
        <section class="mt-4 grid gap-3 md:grid-cols-4">
            <div v-for="system in systems" :key="system.name" class="deco-card !p-4">
                <div class="flex items-center justify-between"><span class="text-sm">{{ system.name }}</span><span class="deco-pill green">{{ system.status }}</span></div>
