...
<template>
    ...
    <div class="deco-page">
        <div class="deco-header">
            <div>
                <h1 class="deco-title">每日官媒运营复盘台</h1>
                <p class="deco-subtitle">
                    以 Instagram 官媒为主，快速复盘每日发布与互动表现。
                </p>
            </div>
        </div>
        <div class="deco-tabs">
        ...
        <template v-if="active === 0">
            <div class="mb-4 flex flex-wrap gap-2">
                <button
                    v-for="item in ['全部', 'Instagram', 'Facebook', 'YouTube']"
                    :key="item"
                    class="deco-button"
                    :class="{ primary: platform === item }"
                    :disabled="loading"
                    @click="loadPage(1, item)"
                >
                    {{ item }}
                </button>
            ...
            <div class="deco-metrics five">
                <article
                    v-for="metric in [
                        { l: '帖子数', v: totals.posts },
                        { l: '浏览量', v: fmt(totals.views) },
                        { l: '点赞', v: fmt(totals.likes) },
                        { l: '评论', v: fmt(totals.comments) },
                        { l: '分享', v: fmt(totals.shares) },
                    ]"
                    :key="metric.l"
            ...
            <div class="deco-grid-2">
                <section class="deco-card">
                    ...
                    >
                        ...
                        <div
                            class="mt-2 h-5 bg-blue-500"
                            :style="{
                                width:
                                    Math.max(
                                        3,
                                        (item.v * 100) /
                                            Math.max(totals.views, 1),
                                    ) + '%',
                            }"
                        ></div>
                    </div>
                ...
                <section class="deco-card">
                    ...
                    <div
                        v-for="item in totals.platforms"
                        :key="item.name"
                        class="mb-4 rounded bg-[#2c3034] p-4"
                    >
                        <div class="flex justify-between">
                            <b class="capitalize">{{ item.name }}</b
                            ><span>互动率 {{ item.rate.toFixed(2) }}%</span>
                        </div>
                        <p class="mt-2 text-xs text-[#9ba3ad]">
                            {{ item.posts }} 帖 · {{ fmt(item.views) }} 浏览 ·
                            {{ fmt(item.eng) }} 互动
                        </p>
                    </div>
                </section>
            ...
            <section class="deco-card">
                <div class="flex items-center justify-between">
                    <h2 class="deco-card-title">帖子明细</h2>
                    <div class="flex gap-2">
                        <button
                            v-for="item in ['可见帖子', '已隐藏帖子']"
                            :key="item"
                            class="deco-button"
                            :class="{ primary: visibility === item }"
                ...
                <p class="mb-3 text-xs text-[#929aa4]">
                    当前显示 {{ rows.length }} 条，共
                    {{ recordPage?.total ?? 0 }} 条
                </p>
        ...
        <section v-else class="deco-card">
            <h2 class="deco-card-title">
                {{ ['', '每日复盘', '周报', 'AI 分析'][active] }}
            </h2>
            <p class="text-sm leading-7 text-[#b6bdc6]">
                本页基于 {{ totals.posts }} 条数据库记录生成。
            </p>
        </section>
    </div>
</template>
