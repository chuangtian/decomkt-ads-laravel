...
const storeGroups = computed(
    () => (page.props.storeNavigation as MenuGroup[] | undefined) ?? [],
);
const globalGroups = computed(
    () => (page.props.globalNavigation as MenuGroup[] | undefined) ?? [],
);
...
const activeGroupTitle = () =>
    allGroups.value.find(groupContainsActive)?.title ?? null;
    ...
    <aside class="macfox-sidebar">
        <nav class="macfox-nav" aria-label="主导航">
            ...
            <section
                ...
                :key="group.title"
                class="macfox-nav-group"
            >
                <button
                    class="macfox-nav-parent"
                    :class="{ active: groupContainsActive(group) }"
                    @click="toggleGroup(group.title)"
                >
                    <component
                        :is="iconMap[group.icon] || LayoutGrid"
                        class="size-[18px]"
                    />
                    <span>{{ group.title }}</span>
                    <ChevronDown
                        class="ml-auto size-4 transition-transform"
                        :class="{ 'rotate-180': expandedGroup === group.title }"
                    />
                </button>
                <div
                    v-show="expandedGroup === group.title"
                    class="macfox-nav-children"
                >
                    <Link
                        ...
                        class="macfox-nav-item"
                        :class="{ active: isActive(item.href) }"
                    >
                        <component
                            :is="iconMap[item.icon] || BriefcaseBusiness"
                            class="size-[18px]"
                        />
                        <span>{{ item.title }}</span>
                    </Link>
                    <p v-if="group.items.length === 0" class="macfox-nav-empty">
                        本店暂未启用子页面
                    </p>
                </div>
            </section>
            ...
            <p v-if="globalGroups.length" class="macfox-nav-section-label">
                组织与系统
            </p>
            <section
